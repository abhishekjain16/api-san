...
class ApiResponsesControllerTest < ActionController::TestCase
  ...
  def test_create_success_with_get_request
    ...
    post :create, params: { url: url, method: 'get' }
  ...
  def test_create_with_post_request_and_request_params
    ...
    post :create, params: { url: url, method: 'post', request_parameters: [{'key' => "some_parameter_key", 'value' => 'some_value'}] }
  ...
  def test_show_success
    ...
    RestClient::Request.expects(:execute).with(url: "http://www.example.com", method: 'get', verify_ssl: false, user: "username", password: "password", :headers => {:content_type => :json}, :payload => {}).returns(response)
    service.process
